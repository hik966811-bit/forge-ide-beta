...
const DEFAULTS = {
  ...
  STABLE_DELAY       : 800,       // 0.8s of silence before response is done
  SEND_DELAY         : 300,       // 0.3s after filling input
  GENERATION_POLL    : 250,       // poll every 250ms
  ...
  SESSION_DIR        : path.join(os.homedir(), '.forge-ide', 'session'),
  ...
  // Memory & Cache
  MEMORY_ENABLED     : true,
  MEMORY_FILE        : path.join(os.homedir(), '.forge-ide', 'memory.json'),
  CACHE_ENABLED      : true,
  CACHE_TTL_MS       : 30_000,
  ...
  HISTORY_FILE       : path.join(os.homedir(), '.forge-ide', 'history.json'),
  HISTORY_MAX_ENTRIES: 200,

  // Plugins
  PLUGIN_DIR         : path.join(os.homedir(), '.forge-ide', 'tools'),

  // Templates
  TEMPLATES_FILE     : path.join(os.homedir(), '.forge-ide', 'templates.json'),
  ...
  SPONSOR_NUDGE_FILE      : path.join(os.homedir(), '.forge-ide', 'sponsor-nudge.json'),
...
const globalConfigPath  = path.join(os.homedir(), '.forge-ide', 'config.json');
...
try {
  ...
  fs.mkdirSync(path.join(os.homedir(), '.forge-ide', 'logs'), { recursive: true });
} catch {}
