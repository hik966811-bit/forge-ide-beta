...
function buildSystemPrompt(profileName = 'default', profileAddition = '') {
  ...
  const lines = [
    ...
    '- Replace the example above with a REAL tool name from AVAILABLE TOOLS below.',
    '- NEVER output placeholder names like TOOL_NAME_HERE, TOOL_NAME, param1, or value1.',
...
class ConversationManager {
   ...
   */
  buildFirstMessage(task, workingDirListing, profileName = 'default', profileAddition = '', conversationHistory = []) {
    ...
    if (conversationHistory && conversationHistory.length > 0) {
      const prev = conversationHistory.slice(-20);
      historyBlock = [
        '',
        'PREVIOUS CONVERSATION IN THIS CHAT:',
        '──────────────────────────────────',
        ...prev.map(m => `${m.role === 'user' ? 'User' : 'Assistant'}: ${m.text}`),
        '',
        '(Use the above context to understand what was discussed before.)',
        '',
      ].join('\n');
    }
