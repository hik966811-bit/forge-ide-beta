...
const BUILT_IN_COMMANDS = [
  ...
  // ── Save config ───────────────────────────────────────────────────────────
  {
    ...
    description: 'Save current session config changes to ~/.forge-ide/config.json',
    usage: '/save-config',
    category: 'Config',
    requiresArg: false,
    ...
    execute: async ({ config }) => {
      try {
        const pluginDir = config.PLUGIN_DIR ||
          path.join(os.homedir(), '.forge-ide', 'tools');
  ...
  // ── MCP ──────────────────────────────────────────────────────────────────
  {
    ...
    requiresArg: false,
    execute: async ({ arg }) => {
      const { getMCPTools, initMCPServers, shutdownMCPServers } = require('./tools');
      const sub = (arg || 'status').toLowerCase().trim();

      if (sub === 'list' || sub === 'status') {
        const servers = config.MCP_SERVERS || [];
        if (servers.length === 0) {
          return 'No MCP servers configured.\nAdd servers to ~/.forge-ide/config.json:\n  "MCP_SERVERS": [{"name": "my-server", "command": "npx", "args": ["-y", "@modelcontextprotocol/server-filesystem"]}]';
        }
        ...
        if (tools.length > 0) {
          ...
          lines.push('\nNot connected. Use /mcp connect to initialize.');
        }
        return lines.join('\n');
      }
      ...
      if (sub === 'connect') {
        ...
        const tools = getMCPTools();
        return `MCP connected. ${tools.length} tools loaded.`;
      }

      if (sub === 'disconnect') {
        await shutdownMCPServers();
        return 'MCP servers disconnected.';
      }

      return 'Usage: /mcp [list|status|connect|disconnect]';
    },
  },
