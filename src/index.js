...
function parseArgs(argv) {
  ...
  const opts = {
    ...
    const a = args[i];
    switch (a) {
      case '--ide':         opts.ide         = true;    break;
      case '--cli':         opts.cli         = true;    break;
...
${c('1;36', 'MORE INFO')}
  Config:  ~/.forge-ide/config.json
  Memory:  ~/.forge-ide/memory.json
  History: ~/.forge-ide/history.json
  Plugins: ~/.forge-ide/tools/
`);
}
...
`);
  ...
  if (args.configPath) {
    const globalConfigPath = path.join(require('os').homedir(), '.forge-ide', 'config.json');
    console.log(globalConfigPath);
    process.exit(0);
  }

  // ── First-time setup auto-initialization ────────────────────────────────
  const globalConfigDir = path.join(require('os').homedir(), '.forge-ide');
  ...
  if (!fs.existsSync(globalConfigPath)) {
    try {
      ...
      fs.writeFileSync(globalConfigPath, JSON.stringify({ MODEL: 'deepseek', WIZARD_COMPLETED: true }, null, 2), 'utf8');
    } catch (e) {}
  }
  ...
  // ── Create agent ───────────────────────────────────────────────────────────
  const ForgeAgent = require('./agent');
  const agent = new ForgeAgent({ saveLog: args.saveLog });
  ...
  // ── Launch IDE mode (Default when running without explicit task, or when --ide is specified) ──
  const shouldLaunchIDE = !args.cli && (args.ide || (!args.task && !args.watch && !args.calibrate && !args.setup && !args.configPath));
  if (shouldLaunchIDE) {
    const IDELauncher = require('./ide-launcher');
    const launcher = new IDELauncher();
    launcher.setAgent(agent);
    logger.banner();
    logger.info('Starting Forge IDE application...');
