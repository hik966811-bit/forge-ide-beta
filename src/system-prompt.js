...
const { TOOLS, getMCPTools } = require('./tools');
...
5. In an existing project, inspect relevant files before modifying them. But for a new project, script, or component in an empty folder, do NOT waste turns reading non-existent files or listing empty directories — immediately create all files needed with write_files!
6. When the ENTIRE task is complete, summarize what you created and conclude with: TASK_COMPLETE
...
function buildToolDocumentation() {
  ...
  // MCP Tools (dynamically loaded from connected servers)
  const mcpTools = getMCPTools();
  if (mcpTools.length > 0) {
    lines.push('MCP Server Tools:');
    mcpTools.forEach(name => {
      const tool = tools[name];
      if (!tool) return;
      const params = Object.entries(tool.parameters || {})
        .map(([k, v]) => `${k}: ${v.type}${v.required ? '' : '?'}`)
