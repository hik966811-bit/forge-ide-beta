...
class DeepSeekAdapter extends BaseAdapter {
    ...
    return withResponseRetry(async () => {
      ...
      while (Date.now() - start < timeout) {
        ...
        if (now - lastIndicatorUpdate > 800) {
          ...
          const thinkingText = (this.thinkingTracker && this.thinkingTracker.thinkingContent) || '';
          logger.thinking(elapsedMs, text.length, thinkingText);
    ...
    return await this.page.evaluate(() => {
      ...
      for (const sel of selectors) {
        try {
          ...
          items.forEach(a => {
            ...
            if (!href.includes('/chat') && !href.includes('/a/chat')) return;
      ...
      if (chats.length === 0) {
        ...
        allLinks.forEach(a => {
          ...
          if (href.includes('/chat') || href.includes('/a/')) {
            seen.add(href);
            const fullUrl = href.startsWith('http') ? href : location.origin + href;
            chats.push({ id: href, title, url: fullUrl });
          }
  ...
  async navigateToChat(chatUrl) {
    try {
      ...
      logger.warn(`Failed to navigate to chat: ${err.message}`);
      return false;
    }
  }
      ...
      for (const sel of selectors) {
        try {
          ...
          items.forEach(el => {
            ...
            const isAssistant = el.className.includes('assistant') || el.className.includes('bot') || el.getAttribute('data-role') === 'assistant';
