...
class ForgeAgent {
  ...
  /** Boot the browser and load Forge */
  async init() {
   ...
   */
  async run(task, templateName = null, conversationHistory = []) {
    ...
    try {
      ...
      if (config.PLANNING_MODE) {
        ...
        const firstMsg   = this.conversation.buildFirstMessage(planPrompt, null, config.ACTIVE_PROFILE, profileAddition, conversationHistory);
        ...
        const firstMsg = this.conversation.buildFirstMessage(task, dirListing, config.ACTIVE_PROFILE, profileAddition, conversationHistory);
        ...
        logger.info('Sending task to Forge...');
      ...
      while (true) {
        ...
        if (parsed.type === 'final' || parsed.type === 'task_complete') {
          ...
          let content = (parsed.content || rawResponse || '').replace(/TASK_COMPLETE/g, '').trim();
          ...
          // If content is empty or was only TASK_COMPLETE, construct a clean descriptive summary
          if (!content) {
            const files = [...progress.filesWritten];
            if (files.length > 0) {
              content = `Task completed successfully.\n\n**Created/modified files:**\n` +
                files.map(f => `- \`${path.basename(f)}\``).join('\n');
            } else if (progress.toolCallCount > 0) {
              content = `Task completed successfully in ${step} step(s).`;
            } else {
              content = 'Task completed successfully.';
            }
          }

          // Always broadcast assistant answer so UI and TUI receive the response
        ...
        // ── Case 4: Plain text / unrecognised type — treat as final answer ──
        //
        // When the AI responds with plain text (not a tool call), treat it as
        // the final conversational answer instead of rejecting it.
        //
        _consecutiveUnrecognised = 0;
        ...
        // Empty response — send correction
        _consecutiveUnrecognised++;
        logger.warn(`Empty response (attempt ${_consecutiveUnrecognised}/${MAX_CORRECTION_ATTEMPTS})`);
        ...
        const correctionMsg = this.conversation.addToolResult(
          'SYSTEM',
          'Your response was empty. Please respond with either a tool call or your answer.',
          true
        );
  ...
  async _saveConversationLog(task, finalResponse) {
    try {
      const os      = require('os');
      const logsDir = path.join(os.homedir(), '.forge-ide', 'logs');
