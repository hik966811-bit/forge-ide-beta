...
const TOOLS = {
  ...
  // ── Batch File Writing ──────────────────────────────────────────────────────
  write_files: {
    description: 'Write (create or overwrite) multiple files in one single tool call. ' +
      'HIGHLY RECOMMENDED for maximum speed when creating multi-file projects, calculators, ' +
      'or components (e.g. index.html, style.css, script.js all at once).',
    parameters: {
      files: {
        type: 'array',
        required: true,
        description: 'Array of file objects: [{ path: "index.html", content: "..." }, { path: "style.css", content: "..." }]',
      },
    },
    async execute({ files }) {
      if (!Array.isArray(files) || files.length === 0) {
        throw new Error('write_files requires a non-empty array of { path, content } objects');
      }
      const results = [];
      for (const item of files) {
        if (!item || !item.path || typeof item.content !== 'string') continue;
        const abs = assertSafePath(item.path, 'write');
        fs.mkdirSync(path.dirname(abs), { recursive: true });
        fs.writeFileSync(abs, item.content, 'utf8');
        const lineCount = item.content.split('\n').length;
        results.push(`✓ ${item.path} (${lineCount} lines, ${formatBytes(Buffer.byteLength(item.content, 'utf8'))})`);
      }
      return `Successfully wrote ${results.length} file(s) in a single batch:\n` + results.join('\n');
    },
  },
...
function appendToAuditLog(entry) {
  ...
  try {
    const logDir = path.join(os.homedir(), '.forge-ide');
...
async function initMCPServers() {
  ...
  try {
    const results = await mcpManager.connectAll(servers);
    for (const r of results) {
      if (r.error) {
        logger.error(`[MCP] ${r.server}: ${r.error}`);
      } else {
        logger.success(`[MCP] ${r.server}: ${r.tools?.length || 0} tools loaded`);
      }
    }
    // Register MCP tools in the TOOLS object
    for (const toolName of mcpManager.getToolNames()) {
      const desc = mcpManager.getToolDescription(toolName);
      const params = mcpManager.getToolParameters(toolName);
      TOOLS[toolName] = {
    ...
    logger.error(`[MCP] Init failed: ${err.message}`);
  }
}
...
function getMCPTools() {
  return mcpManager.getToolNames();
}

module.exports = { TOOLS, executeTool, getToolDescriptions, getLoadedPluginCount, getLoadedPlugins, cache, initMCPServers, shutdownMCPServers, getMCPTools };
